...
class SalesDataSource {
    ...
    async signInAgent() {
        let response = await fetch(`${restUri}/agents/${this.agentLogin.id}/${this.agentLogin.passcode}`);
        if (response.ok) {
    ...
    async readOrders() {
        ...
        let response = await fetch(`${restUri}/orders/${this.orderEntry.customerId}`, request);
        if (response.ok) {
    ...
    async createOrder() {
        ...
        let response = await fetch(`${restUri}/orders`, request);
        if (response.ok) {
